...
_Result = collections.namedtuple('_Result', ['request_text', 'response_text', 'response_audio', 'follow_on'])
...
class GenericSpeechRequest(object):
    ...
    def __init__(self, api_host, credentials, device):
    ...
    def do_request(self, textquery=None):
        ...
        try:
            ...
            if not textquery:
                ...
                response_stream = self._create_response_stream(
                    service, self._text_request(textquery), self.DEADLINE_SECS)
...
class AssistantSpeechRequest(GenericSpeechRequest):
    ...
    def __init__(self, credentials, device):
        ...
        super().__init__('embeddedassistant.googleapis.com', credentials, device)
    ...
    def _create_config_request(self):
        ...
        if self._conversation_state:
            dialog_state_in.conversation_state = self._conversation_state
        device_config=embedded_assistant_pb2.DeviceConfig(
            device_id=self._device['id'],
            device_model_id=self._device['model_id']
        )
    ...
    def _text_request(self, textquery):
        dialog_state_in = embedded_assistant_pb2.DialogStateIn(
            language_code='en-US',
            conversation_state=b''
        )
    ...
    def _handle_response(self, resp):
        ...
        if resp.dialog_state_out.supplemental_display_text:
            self.display_text = resp.dialog_state_out.supplemental_display_text
    ...
    def _finish_request(self):
        ...
        return _Result(self._transcript, self.display_text, self._response_audio, self.dialog_follow_on)
